...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 2) Coleta e normalização necessária para montar o número de contrato
    $id_usuario    = $_SESSION['id_usuario'];
    $data_vistoria = $_POST['data_vistoria'];

    $prefixo = isset($_POST['numero_contrato_prefixo']) ? preg_replace('/\D/', '', $_POST['numero_contrato_prefixo']) : '';
    $ano     = isset($_POST['numero_contrato_ano'])     ? preg_replace('/\D/', '', $_POST['numero_contrato_ano'])     : '';
    if (strlen($prefixo) !== 3 || strlen($ano) !== 4) {
        echo "<script>alert('Número do contrato inválido. Use 000/2025'); window.history.back();</script>";
        exit;
    }
    $numero_contrato = $prefixo . '/' . $ano;

    // 3) Verificação de duplicidade AGORA que $numero_contrato existe
    $check_nc = mysqli_prepare($conexao, "SELECT 1 FROM iniciativas WHERE numero_contrato = ?");
    mysqli_stmt_bind_param($check_nc, "s", $numero_contrato);
    mysqli_stmt_execute($check_nc);
    mysqli_stmt_store_result($check_nc);
    if (mysqli_stmt_num_rows($check_nc) > 0) {
        mysqli_stmt_close($check_nc);
        echo "<script>alert('Já existe uma iniciativa com o Nº de contrato $numero_contrato.'); window.history.back();</script>";
        exit;
    }
    mysqli_stmt_close($check_nc);
    ...
    $ib_valor_medio_raw = $_POST['ib_valor_medio'] ?? '';
    $ib_valor_medio = $ib_valor_medio_raw !== '' ? str_replace(['.', ','], ['', '.'], $ib_valor_medio_raw) : null;
    ...
    if (!$ok && mysqli_errno($conexao) == 1062) {
        // Duplicidade capturada mesmo com corrida entre checagem e insert
    ...
    if ($ok) {
        ...
        $err = addslashes(mysqli_error($conexao));
        echo "<script>alert('Erro ao salvar: {$err}'); window.history.back();</script>";
        exit;
    }
}
?>
